document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadRoomList() {
    try {
      ...
      if (response.ok) {
        ...
        // 僅取公開房間
        allRooms = rooms.filter(room => room.roomType === "public");
  ...
  function displayRooms(rooms) {
    ...
    if (rooms.length === 0) {
      roomListElement.innerHTML = `<p class="no-room">沒有找到符合的公開房間</p>`;
      return;
    }
  ...
  async function joinRoom(roomId) {
    ...
    const joinUrl = `/api/join-room?roomId=${roomId}&playerName=${encodeURIComponent(playerName)}`;
    try {
