...
/* 卡牌定位 (同原本) */
const positions=[{top:'3%',left:'55%'},{top:'3%',right:'55%'},{top:'40%',left:'20%'},{top:'40%',right:'20%'},{bottom:'30px',left:'50%',transform:'translateX(-50%)'}];
...
function renderPlayers(arr){
  ...
  /* 底部按鈕顯示／隱藏 */
  const leaderBox=document.getElementById("leader-action");
  if(leaderBox) leaderBox.classList.toggle("hidden", leaderId!==playerName);
}
...
async function fetchPlayers(){
  try{
    ...
    players=await res.json();window.players=players;renderPlayers(players);
...
document.addEventListener("DOMContentLoaded",async()=>{
  ...
  /* 底部按鈕事件繫結 */
  const leaderActionBtn=document.getElementById("select-expedition-btn");
  leaderActionBtn.addEventListener("click",openSelectModal);
  connectWebSocket();
  bindChatIcon();
});
